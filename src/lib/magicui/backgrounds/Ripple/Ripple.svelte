<script lang="ts">
  ...
  export let numCircles: number = 5;
</script>
...
>
  {#each { length: numCircles } as _, i}
    <div
      class="absolute animate-ripple rounded-full bg-foreground/25 shadow-xl border top-1/2 left-1/2 translate-x-1/2 translate-y-1/2 [--i:{i}]"
      style="width: {mainCircleSize + i * 70}px; height: {mainCircleSize +
        i * 70}px; opacity: {mainCircleOpacity -
        i * 0.03}; animation-delay: {i * 0.06}s; border-style:{i ===
      numCircles - 1
        ? 'dashed'

<script lang="ts">
    ...
    onMount(() => {
        if (!dev) {
            ...
            return () => {
                const scriptNode = container?.querySelector(
                    `[data-id="${localId}"]`
                );
    ...
    beforeNavigate((navigation) => {
        ...
        const isDocIndex =
            navigation.from?.route.id === `/${pathname}/${fromparam}`;
        if (isDocIndex) return;
        const goingToDocIndex =
            navigation.to?.route.id === `/${pathname}/${toparam}`;
